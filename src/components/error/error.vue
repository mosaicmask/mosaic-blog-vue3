...
<style lang="scss" scoped>
  ...
  * {
  ...
  .skyscrappers__list {
    width: 100%;
    height: calc(calc($crane-height / 2));
    left: 0;
    .skyscrapper__item {
      position: absolute;
      height: inherit;
      ...
      background: linear-gradient(
        115deg,
        $c-malabars 73%,
        darken($c-malabars, 10%) 73%,
        darken($c-malabars, 10%) 100%
      );
      &::after {
        content: '';
        position: absolute;
      ...
      &:last-child:not(:only-child) {
        // Dynamic last-child pseudo-class selector (if it's desired to add new buildings)
        background: $c-malabars;
      }
    }
    @for $i from 1 through $city-skyscrappers {
      .skyscrapper-#{$i} {
        width: calc($crane-height / (calc($i / 0.7))); //Building's width
        height: calc($crane-height / (calc($i / 0.8))); //Building's height
        right: (15 + ($i * 10)) *
          1%; // Building positioning and workaround to add units to SCSS calculation
        bottom: 0;
        z-index: 10;
  ...
  @for $i from 1 through $city-cranes {
    .crane-#{$i} {
      ...
      @if $i == 2 {
        bottom: -1rem;
        z-index: -1;
        transform: scale(0.75) scaleX(-1); // Mirror and scale effect to simulate a far crane
        @media screen and (max-width: $media900) {
          display: none;
        }
      }
  ...
  .tree__container {
    width: 100%;
    height: calc(calc($crane-height / 2) - calc($trunk-height * 3));
    left: 0;
